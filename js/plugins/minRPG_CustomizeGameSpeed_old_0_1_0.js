...
/*:
 * @plugindesc ツクールのゲームスピードを快適に遊べるように調整されたプラグインです。詳細はヘルプを見てね。
 ...
 *
 * 【概要】
 * ツクールMVサンプルゲーム「Sea Pirate シーピラート」のプロジェクトファイル
 * に入っている凄腕プラグイン「Tachi.js」を改良して、
 * ゲームスピードを高速化し、ユーザさんがカスタマイズできるようにしたものです。
 * 
 * 【プラグインコマンド】
 ...
 * 
 * 【主な機能】
 * このプラグインをONにすると、ゲーム中の様々な動作が快適になります。
 * 
 * ・ゲームスピードの高速化（マップ・戦闘）
 * ・スキップモードの追加（デフォルトではダッシュボタン「Shift」キーに割り当て）
 * 
 * Tachiさん「Tachi.js」で、プレイヤーがより快適に遊べるように、
 * 何度も何度もテストプレイをされて、ゲームスピードを調整されていました。
 * 「なんだか戦闘エフェクトの待ち時間が多いな」とか、
 * 「もう少しだけここを早くできないかな」とか、
 * ユーザさんがストレスを感じやすいウェイトを、極力少なくしてあります。
 * 
 * そのノウハウを、なんとか多くのツクール製ゲームに応用できないかと思い、
 * このプラグインを作成しました。
 * タッチ操作にも対応しています。
 * （ただし、実機でのテストはしてません。誰か助けて…＞＜）。
 *
 * 【その他の機能について】
 * この他にも、ユーザさんがゲームをより柔軟にコントロールできるように、
 * プラグインのパラメータをたくさん用意しています。
 * もちろん、デフォルトのままでも、快適に遊べるようにしていますが、
 * 「もっと調整したい」という人向けに、
 ...
 *
 * 
 * 【背景】 → 各種デフォルト機能の問題点と解決策について書いています。
 * 
 * ＜ボタン長押しによる早送りの問題点＞
 ...
 * 
 * 
 *  ・パラメータ「機能:戦闘全滅後もゲームを継続するか」
 *    がONの時、下記メソッドを上書き
 *     BattleManager.updateBattleEnd
 *       → ゲームオーバー制御のため。
 *        （ランダムエンカウントの敵に負けても、ゲームオーバーにせず、
 *          指定したスイッチをONにして、エディタで検出可能にする機能）
 * 
 *  ・パラメータ「機能:ランダムエンカウント時にスイッチをONにするか」
 *    がONの時、下記メソッドを上書き
 ...
 * @default 
 * 
 * @param 機能:戦闘全滅後もゲームを継続するか
 * @desc 初期値=OFF。競合対策。ONだと、指定したスイッチがONになるだけです。他のプラグインを使用する場合はOFFにしてください。
 * @default OFF
 * 
 * @param 機能:ランダムエンカウント時にスイッチをONにするか
 * @desc 初期値=OFF。競合対策。ONだと、指定したスイッチをONにし、変数に敵グループIDを格納します。他のプラグインを使用する場合はOFFにしてください。
 * @default OFF
 * 
 * @param 機能:吸収技もダメージエフェクトをつけるか
 * @desc 初期値=ON。機能:吸収技もダメージエフェクトをつけるかです。他のプラグインを使用する場合は、OFFにしてください。
 * @default ON
 * 
...
(function () {
    ...
    */
    ...
    // 機能:戦闘全滅後もゲームを継続するかのON/OFF
    var _isOnContinueBattleEnd = getParamBoolean(['isOnContinueBattleEnd', '機能:戦闘全滅後もゲームを継続するか']);
    // 機能:ランダムエンカウント時にスイッチをONにするかのON/OFF
    var _isOnSwitchBattleEncount = getParamBoolean(['isOnSwitchBattleEncount', '機能:ランダムエンカウント時にスイッチをONにするか']);
    ...
    // 可変なゲームスピードを持つ変数番号
    var _VarNo_BattleSpeed = getParamNumber(['VarNo_BattleSpeed', '可変戦闘スピードを格納する変数番号'], 0, 9999);
    var _VarNo_MessageWaitEscapeCharacterSpeed = getParamNumber(['VarNo_MessageWaitEscapeCharacterSpeed', '可変メッセージウェイト制御文字スピードを格納する変数番号'], 1, 9999);
    var _VarNo_SkipRate = getParamNumber(['VarNo_SkipRate', '可変スキップ倍数を格納する変数番号'], 1, 9999);
    
    // ランダムエンカウント時、敵グループIDを格納する変数番号
    var _VarNo_RandomEncountTroopID = getParamNumber(['VarNo_RandomEncountTroopID', 'ランダムエンカウント敵グループIDを格納する変数番号'], 1, 9999);
    // ランダムエンカウント時にONになるスイッチ番号
    var _SwitchNo_RandomEncount = getParamNumber(['SwitchNo_RandomEncount', 'ランダムエンカウント時にONになるスイッチ番号'], 1, 9999);
    // 戦闘勝利時、全滅時にONになるスイッチ番号
    var _SwitchNo_BattleWin = getParamNumber(['SwitchNo_BattleWin', '戦闘勝利時にONになるスイッチ番号'], 1, 9999);
    var _SwitchNo_BattleLose = getParamNumber(['SwitchNo_BattleLose', '戦闘全滅時にONになるスイッチ番号'], 1, 9999);
    // 各種、早送りボタンを機能させるかの有無
    ...
    var _VarNo_Migawari_Sareta_AcorID = getParamNumber(['VarNo_Migawari_Sareta_AcorID', '最後にかばわれたアクターIDを格納する変数番号'], 1, 9999);
    var _VarNo_Migawari_Shita_AcorID = getParamNumber(['VarNo_Migawari_Shita_AcorID', '最後にかばったアクターIDを格納する変数番号'], 1, 9999);
    var _VarNo_Migawari_Sareta_EnemyID = getParamNumber(['VarNo_Migawari_Sareta_EnemyID', '最後にかばわれた敵キャラIDを格納する変数番号'], 1, 9999);
    var _VarNo_Migawari_Shita_EnemyID = getParamNumber(['VarNo_Migawari_Shita_EnemyID', '最後にかばった敵キャラIDを格納する変数番号'], 1, 9999);
    // 身代わり成功した（かばった）瞬間だけ実行されるコモンイベント番号。0は無視（何も実行しない）を意味する。ツクールコモンイベント番号は、1～1000までのはず。
    var _CommonEventNo_Migawari_Success_Actor = getParamNumber(['CommonEventNo_Migawari_Success_Actor', '味方の身代わり成功時に呼び出すコモンイベント番号'], 1, 1000);
    var _CommonEventNo_Migawari_Success_Enemy = getParamNumber(['CommonEventNo_Migawari_Success_Enemy', '敵の身代わり成功時に呼び出すコモンイベント番号'], 1, 1000);
    ...
    */
    ...
    Game_BattlerBase.prototype.paramMax = function (paramId) {
        if(_isOnChangeParamLimit_inThisPlugin === false){              // このプラグインのステータス限界値変更を禁止するなら、
    ...
    Scene_Map.prototype.updateMainMultiply = function () {        // 元メソッドにreturnはないよ。
        if(_isOnChangeSpeed_inThisPlugin === false){               // このプラグインのスピード変更を禁止するなら、
    ...
    /**  Scene_Map.prototype.isFastForwardの上書き。早送りモードに移行する条件の追加。
      ...
      * 
      * ※プラグインパラメータ「機能:スピード変更」がONだと、上記の効果は無効となり、元のメソッドを呼び出しますので、注意してください。
    */
            ...
            if (this.isBattleTest()) {
                ...
                if (this._canLose) {
                    ...
                    if (_SwitchNo_BattleLose != null && _SwitchNo_BattleLose >= 1 && _SwitchNo_BattleLose < $dataSystem.switches.length) {
                        ...
                        alert('minRPG_CustomizeGameSpeed: パラメータ「戦闘全滅時にONになるスイッチ番号」に変な値が入ってます。: ' + _SwitchNo_BattleLose);
                    }
                    $gameParty.reviveBattleMembers();
                    SceneManager.pop();
                }
                ...
                if (_SwitchNo_BattleWin != null && _SwitchNo_BattleWin >= 1 && _SwitchNo_BattleWin < $dataSystem.variables.length) {
                    ...
                    alert('minRPG_CustomizeGameSpeed: パラメータ「戦闘勝利時にONになるスイッチ番号」に変な値が入ってます。: ' + _SwitchNo_BattleWin);
                }
                SceneManager.pop();
            }
            this._phase = null;
    ...
    Game_Player.prototype.executeEncounter = function () {      // 元メソッドは、エンカウントしたかを返すよ。
        if(_isOnSwitchBattleEncount === false){                 // このプラグインのエンカウント検知機能をつけるパラメータがOFFなら、元のメソッドを呼び出すだけだよ（競合対策）。
            return  _Game_Player_executeEncounter.apply(this, arguments); // .call(this, 引数1, 引数2, ...)よりapplyの方がコピペミスを防げていいよ。
        }else{
            // このプラグインの「エンカウント検知機能をつける」パラメータがONなら、元メソッドは呼び出さず、下の処理に置き換えるよ
            ...
            if (!$gameMap.isEventRunning() && this._encounterCount <= 0) {
                ...
                if ($dataTroops[troopId]) {
                    ...
                    if (_SwitchNo_RandomEncount != null && _SwitchNo_RandomEncount >= 1 && _SwitchNo_RandomEncount < $dataSystem.switches.length) {
                        ...
                        alert('minRPG_CustomizeGameSpeed: パラメータ「ランダムエンカウント時にONになるスイッチ番号」に変な値が入っています。: ' + _SwitchNo_RandomEncount);
                    }
                    // 変数番号を格納している、パラメータに変な値が入っていないか、随時チェック。
                    if (_VarNo_RandomEncountTroopID != null && _VarNo_RandomEncountTroopID >= 1 && _VarNo_RandomEncountTroopID < $dataSystem.variables.length) {
                        ...
                        alert('minRPG_CustomizeGameSpeed: パラメータ「ランダムエンカウント敵グループIDを格納する変数番号」に変な値が入っています。: ' + _VarNo_RandomEncountTroopID);
                    }
    ...
    BattleManager.checkSubstitute = function (target) {
        ...
        if(_isOnChangeDefault_Migawari === false){
    ...
    BattleManager.applySubstitute = function (target) {
        ...
        // 以下がデフォルトからの変更点。
        // パラメータ_isOnChangeDefault_Migawariがfalseだったら、デフォルトのものを呼び出すよ。
        if(_isOnChangeDefault_Migawari === false){
                                    ...
                                    if (Math.random() * 100 <= _Migawawri_Suru_ActPercent) {
                                        ...
                                        if(_isDamagedCharaActor){  // 敵なのか、味方なのかで、格納する変数を分岐
                                            ...
                                            if (_VarNo_Migawari_Sareta_AcorID != null && _VarNo_Migawari_Sareta_AcorID >= 1 && _VarNo_Migawari_Sareta_AcorID < $dataSystem.variables.length) {
                                                ...
                                                alert('minRPG_CustomizeGameSpeed: パラメータ「身代わりされたアクターIDを格納する変数番号」に変な値が入っています。: ' + _VarNo_Migawari_Sareta_AcorID);
                                            }
                                            // 変数番号を格納している、パラメータに変な値が入っていないか、随時チェック。
                                            if (_VarNo_Migawari_Shita_AcorID != null && _VarNo_Migawari_Shita_AcorID >= 1 && _VarNo_Migawari_Shita_AcorID < $dataSystem.variables.length) {
                                                ...
                                                alert('minRPG_CustomizeGameSpeed: パラメータ「身代わりしたアクターIDを格納する変数番号」に変な値が入っています。: ' + _VarNo_Migawari_Shita_AcorID);
                                            }
                                            ...
                                            if (_commonEventNo != null && _commonEventNo >= 1 && _commonEventNo < $dataCommonEvents.length) {
                                                ...
                                                alert('minRPG_CustomizeGameSpeed: パラメータ「味方の身代わり成功時に呼び出すコモンイベント番号」に変な値が入っています。: ' + _CommonEventNo_Migawari_Success_Actor);
                                            }
                                        }else{
                                            // 変数番号を格納している、パラメータに変な値が入っていないか、随時チェック。
                                            if (_VarNo_Migawari_Sareta_EnemyID != null && _VarNo_Migawari_Sareta_EnemyID >= 1 && _VarNo_Migawari_Sareta_EnemyID < $dataSystem.variables.length) {
                                                ...
                                                alert('minRPG_CustomizeGameSpeed: パラメータ「身代わりされた敵キャラIDを格納する変数番号」に変な値が入っています。: ' + _VarNo_Migawari_Sareta_EnemyID);
                                            }
                                            // 変数番号を格納している、パラメータに変な値が入っていないか、随時チェック。
                                            if (_VarNo_Migawari_Shita_EnemyID != null && _VarNo_Migawari_Shita_EnemyID >= 1 && _VarNo_Migawari_Shita_EnemyID < $dataSystem.variables.length) {
                                                ...
                                                alert('minRPG_CustomizeGameSpeed: パラメータ「身代わりした敵キャラIDを格納する変数番号」に変な値が入っています。: ' + _VarNo_Migawari_Shita_EnemyID);
                                            }
                                            ...
                                            if (_commonEventNo != null && _commonEventNo >= 1 && _commonEventNo < $dataCommonEvents.length) {
                                                ...
                                                alert('minRPG_CustomizeGameSpeed: パラメータ「敵の身代わり成功時に呼び出すコモンイベント番号」に変な値が入っています。: ' + _CommonEventNo_Migawari_Success_Enemy);
                                            }
                                        }
        ...
        switch (code) {
            case '.':
                if(isSkipModeOn() === false){                                   // スキップ中だと、待ちません。
                    ...
                    if(_isDebugMode()){
                       console.log('minRPG_CustomizeGameSpeed.js 「\\.」や「\\|」のウェイトフレーム数の変更: _WaitPeriodDef:'+_WaitPeriodDef +' → 変換後:'+_waitFrameNum);
                    }
